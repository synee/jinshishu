...
from app.forms.angular_base import NgFormBaseMixin, SafeTuple
...
class NgModelFormMixin(NgFormBaseMixin):
    ...
    def get_widget_attrs(self, bound_field):
        ...
        ng = {
            ...
            'model': "form.%s" % (self.scope_prefix and ('%s.%s' % (self.scope_prefix, identifier)) or identifier)
        }
        attrs = {}
        if hasattr(self, 'Meta') and bound_field.name in getattr(self.Meta, 'ng_models', []):
            attrs['ng-model'] = "form.%s" % ng['model']
